...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-v", "--verbose", help="Enable output to screen", action="store_true")
    parser.add_argument(
    ...
    if args.write:
      ...
      with open('tcp.json', 'w', encoding='utf-8') as f:
        json.dump(result_tcp, f, ensure_ascii=False, indent=4)

      with open('udp.json', 'w', encoding='utf-8') as f:
        json.dump(result_udp, f, ensure_ascii=False, indent=4)

    if args.test:
        logging.debug("*****[TEST MODE ENABLED]*****")
        with open('tcp.json') as f:
          result_tcp = json.load(f)

        with open('udp.json') as f:
          result_udp = json.load(f)
